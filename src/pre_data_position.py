...
# Lang()
    ...
    def add_sen_to_vocab(self, sentence, output = False):  # add words of sentence to vocab
        for word in sentence:
            if re.search("N_\d+|NUM|V_\d+|\d+", word):
                continue
    ...
    def build_output_lang(self, generate_num, copy_nums):  # build the output lang vocab and dict
        self.index2word = ["PAD", "EOS"] + self.index2word + generate_num + ["N_" + str(i) for i in range(copy_nums)] +\
                          ["SOS", "UNK"]
        self.n_words = len(self.index2word)
        for i, j in enumerate(self.index2word):
            self.word2index[j] = i
    ...
    def build_output_lang_for_tree(self, generate_num, copy_nums):  # build the output lang vocab and dict
        ...
        self.index2word = self.index2word + generate_num + ["N_" + str(i) for i in range(copy_nums)] + ["UNK"]
        self.n_words = len(self.index2word)
...
def transfer_num(data):  # transfer num into "NUM"
    ...
    # 在这里生成了每个文本数据的依赖
    #all_dep_info = load_depfile(os.path.join("data",  '{}.txt.dep'.format("Math_23K")))
...
def transfer_num_question(data):  # transfer num into "NUM"
    print("Transfer numbers...")
    pattern = re.compile("\d*\(\d+/\d+\)\d*|\d+\.\d+%?|\d+%?")
    pairs = []
    generate_nums = []
    ...
    for d in data:
        ...
        if config.USE_APE:
            ...
            toFind = ['？', '多少']
            assert seg[-1] in toFind 
            #print(list(filter(lambda x: seg[x]==toFind, list(range(len(seg))))))
            equations = d["equation"][2:]
...
def prepare_data(tokenizer, pairs_trained, pairs_tested_list, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    for pair in pairs_trained:
        ...
        len_input = len(input_cell)

        assert(len_in == len_input)
    ...
    for pairs_tested in pairs_tested_list:
        ...
        for pair in pairs_tested:
            ...
            #print("len of pair[0]:{}".format(len(pair[0])))
            input_cell = indeces_from_sentence_via_tokenizer(tokenizer, pair[0])
            #print("len of input_cell:{}".format(len(input_cell)))
...
def prepare_data_prog(tokenizer, pairs_trained, pairs_tested_list, trim_min_count, generate_nums, copy_nums, tree=False):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
...
def prepare_train_batch_prog(pairs_to_batch, batch_size):
    pairs = copy.deepcopy(pairs_to_batch)
    random.shuffle(pairs)  # shuffle the pairs
    pos = 0
    input_lengths = []
    ...
    for batch in batches:
        ...
        for i, li, j, lj, num, num_pos, num_stack, _, _ in batch:   #, num_gravity
            num_batch.append(len(num))
            input_batch.append(pad_seq(i, li, input_len_max))
            output_batch.append(pad_seq(j, lj, output_len_max))
