...
class EncoderSeq_noVAE(nn.Module):
    ...
    def forward(self, input_seqs, input_lengths, mask=None):
...
class EncoderSeq_Gru(nn.Module):
    def __init__(self, input_size, vocab_size, hidden_size, n_layers=2, dropout=config.dropout, embedding_size=768):
        super(EncoderSeq_Gru, self).__init__()
    ...
    def forward(self, input_seqs, input_lengths, mask=None, hidden=None, embedding=None):

        float_mask = 1 - mask.float()
        embedded = embedding(input_seqs, attention_mask = float_mask)[0].transpose(0,1)  # S x B x E
        ...
        # print("pade_outputs:{}".format(pade_outputs.shape))
        # print("pade_hidden:{}".format(pade_hidden.shape))
        problem_output = pade_outputs[-1, :, :self.hidden_size] + pade_outputs[0, :, self.hidden_size:]
        pade_outputs = pade_outputs[:, :, :self.hidden_size] + pade_outputs[:, :, self.hidden_size:]  # S x B x H
        return pade_outputs, problem_output#pade_outputs, pade_hidden, problem_output
