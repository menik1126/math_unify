...
def train_attn_prog(input_batch, input_length, target_batch, target_length, 
    ...
    # 这样初始化
    decoder_hidden = torch.stack([problem_output, problem_output], dim=0)#encoder_hidden[:decoder.n_layers]  # Use last (forward) hidden state from encoder
    ...
    loss = masked_cross_entropy_comp_noVAE(
        all_decoder_outputs.transpose(0, 1).contiguous(),  # -> batch x seq
        target.transpose(0, 1).contiguous(),  # -> batch x seq
        target_length
    )
...
def evaluate_attn(input_seq, input_length, num_list, copy_nums, generate_nums, encoder, decoder, output_lang,
    ...
    # Run through encoder
    encoder_outputs, problem_output = encoder(input_var, [input_length], None)
...
def evaluate_attn_prog(input_seq, input_length, 
                  copy_nums, generate_nums, encoder, decoder, output_lang,
                  beam_size=1, english=False, max_length=MAX_OUTPUT_LENGTH):
    seq_mask = torch.BoolTensor(1, input_length).fill_(0)
    num_start = output_lang.n_words - copy_nums - 2
    ...
    # Create starting vectors for decoder
    decoder_input = torch.LongTensor([output_lang.word2index["SOS"]])  # SOS
    decoder_hidden = torch.stack([problem_output, problem_output], dim=0)  # Use last (forward) hidden state from encoder
    beam_list = list()
    score = 0
    beam_list.append(Beam(score, decoder_input, decoder_hidden, []))
...
def train_tree_SWS_divide_unsym(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
    ...
    loss_no_prue, kl_loss1, vae_kl1 = train_prune_SWS_divide(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
                                ...
                                encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer,
                                encoder1, predict1, generate1, merge1,
                                pos = 0, prune_start = False, is_no_mask = True)
    # unsym0 是True False
    # unsym1 是False True
    loss_prue, kl_loss2, vae_kl2 = train_prune_SWS_divide(input_batch, input_length, target_batch, target_length, nums_stack_batch_copy, num_size_batch, num_pos, generate_nums, output_lang,
                                ...
                                encoder_optimizer1, predict_optimizer1, generate_optimizer1, merge_optimizer1,
                                encoder, predict, generate, merge,
                                pos = 1, prune_start = False, is_no_mask = False)
...
def train_prune_SWS_divide(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
    ...
    with torch.no_grad():
        ...
        # branch with pruned network
        all_node_outputs2, target_noGrad, mean_noGrad, logvar_noGrad = train_one_batch_vae(input_batch[pos], input_length[pos], target_batch[pos], target_length[pos], nums_stack_batch[pos], num_size_batch[pos], num_pos[pos], generate_nums, output_lang,
                                        encoder1, predict1, generate1, merge1) 
        all_node_outputs2_noGrad = torch.stack(all_node_outputs2, dim=1).detach()
    ...
    # if config.is_vmf:
    #    loss, kl_loss, vae_loss = masked_cross_entropy(all_node_outputs1, target, target_length[1-pos], logits_noGrad= all_node_outputs2_noGrad, target_noGrad = target_noGrad, length_noGrad = target_length[pos], temperature = config.temperature, mean=mean, logvar=logvar, problem_output_sample=problem_output_sample ,problem_output_sample_noGrad = problem_output_sample_noGrad)
    # else:
    loss, kl_loss, vae_loss = masked_cross_entropy(all_node_outputs1, target, target_length[1-pos], logits_noGrad= all_node_outputs2_noGrad, target_noGrad = target_noGrad, length_noGrad = target_length[pos], temperature = config.temperature, mean=mean, logvar=logvar, is_loss_no_mask = is_no_mask)
...
def train_tree_gru(input_batch, input_length, target_batch, target_length, 
               nums_stack_batch, num_size_batch, 
               generate_nums, encoder, predict, generate, merge, embedding,
    ...
    # Run words through encoder
    encoder_outputs, problem_output = encoder(input_var, input_length, mask = seq_mask, embedding = embedding)
...
def train_tree_comp(input_batch, input_length, target_batch, target_length, 
    ...
    loss, vae_kl = masked_cross_entropy_comp_noVAE(all_node_outputs, target, target_length, mean=mean, logvar = logvar)
    # loss = loss_0 + loss_1
    loss.backward()
    ...
    return loss.item(), vae_kl.item()  # , loss_0.item(), loss_1.item()
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos,
    ...
    if USE_CUDA:
        ...
        num_mask = num_mask.cuda()
    # Run words through encoder
    print("input_var:{} seq_mask:{}".format(input_var.shape, seq_mask.shape))
    if config.is_mask:
        encoder_outputs, problem_output= encoder(input_var, [input_length], mask = seq_mask)#, problem_output_sample, _, _ = encoder(input_var, [input_length], mask = seq_mask)#, problem_output_sample, _, _ = encoder(input_var, [input_length], mask = seq_mask)
    else:
        encoder_outputs, problem_output = encoder(input_var, [input_length])#, problem_output_sample, _, _ = encoder(input_var, [input_length])
...
def evaluate_tree_gru(input_batch, input_length, generate_nums, 
    ...
    seq_mask = torch.BoolTensor(1, input_length).fill_(0)
    # Turn padded arrays into (batch_size x max_len) tensors, transpose into (max_len x batch_size)
    input_var = torch.LongTensor(input_batch).unsqueeze(1).transpose(0, 1)
    ...
    #print("input_var:{} seq_mask:{}".format(input_var.shape, seq_mask.shape))
    if config.is_mask:
        encoder_outputs, problem_output= encoder(input_var, [input_length], mask = seq_mask, embedding=embedding)#, problem_output_sample, _, _ = encoder(input_var, [input_length], mask = seq_mask)#, problem_output_sample, _, _ = encoder(input_var, [input_length], mask = seq_mask)
    else:
        encoder_outputs, problem_output = encoder(input_var, [input_length], embedding=embedding)#, problem_output_sample, _, _ = encoder(input_var, [input_length])
